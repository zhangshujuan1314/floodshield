...
| 中间件 | 5 个（限流、安全头、净化、大小限制、请求ID） |
| 数据适配器 | 4 组（气象、地图、通知、AI） |
| **安卓 APP** | **53 个文件，5 页面 + 5 组件** |
| 微信小程序 | 6 页面 |
| 管理后台 | 8 页面 |
| 对抗审查 | 7 轮，0 FAIL 残留 |
...
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│  安卓原生APP  │  │  微信小程序   │  │  管理后台     │
│ Kotlin+Compose│  │ Taro+React   │  │ Next.js      │
│ 5页面+5组件   │  │ 6页面        │  │ 8页面        │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       │                 │                 │
       └────────────┬────┴─────────────────┘
...
|------|------|
| 首页 | 风险仪表盘、预警卡片、快捷操作 |
| 地图 | Google Maps + 图层切换 + 标记 |
| 避险所 | 按距离排序、容量条、无障碍筛选 |
| 上报 | 表单 + 拍照 + 定位自动检测 |
| 我的 | 通知偏好、语言、订阅区域 |

**组件：** RiskBadge（风险徽章）、AlertCard（预警卡）、ShelterCard（避险所卡）、DataFreshness（数据新鲜度）、VoiceButton（语音播报）
...
floodshield/
├── apps/
│   ├── android/              # Kotlin + Jetpack Compose 安卓原生 APP
│   │   ├── app/
│   │   │   └── src/main/java/com/floodshield/app/
│   │   │       ├── ui/       # 界面层（5 页面 + 5 组件）
│   │   │       ├── data/     # 数据层（API + Repository + Room）
│   │   │       ├── service/  # 定位 + 推送服务
│   │   │       └── di/       # Hilt 依赖注入
│   │   └── build.gradle.kts
...
│       ├── app/
│       │   ├── api/v1/       # API 端点（36 个）
│       │   ├── providers/    # 外部数据适配器（4 组）
│       │   ├── middleware/    # 安全中间件（5 个）
│       │   ├── services/     # 风险引擎
│       │   ├── models/       # 数据模型（15 张表）
│       │   └── core/         # 配置、认证、安全
│       ├── tests/            # 测试（147+ 个）
